...
public class DataBaseHelper extends SQLiteOpenHelper implements Config {
    ...
    public float getTransSize(String organism_name){
        float genome_size;
        int kingdom_id;
        float transcription;
        SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
        ...
        try {
            genome_size = organism.getFloat(organism.getColumnIndex("genome_size"));
        ...
        kingdom.moveToFirst();
        try {
            transcription = kingdom.getFloat(kingdom.getColumnIndex("transcription"));
        }catch(IndexOutOfBoundsException e){
            return -1;
        }
        return genome_size*transcription;
    }
    public float getReadSizeMultiplier(float fold_change, int replicates){
        SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
        ...
        Cursor c = qb.query(myDataBase, projection, selection, null, null, null, null);
        c.moveToFirst();
        return c.getFloat(c.getColumnIndex("read_size_multiplier"));
    }
}